...
try:
    ...
    print(f"Fatal! Source Broken. Please implement module 'models' or  'data_loader' or 'Strategies' or 'engine'. Error! {e}. ")
...
class PerformanceReporter:
    ...
    def calculate_metrics(self) -> Dict[str, str]:
        ...
        # 3. Sharpe Ratio
        # (Robust Version) Handle the case where the standard deviation is zero (no volatility)
        if np.std(returns) > 0:
    ...
    def _generate_ascii_plot(self, width: int = 80, height: int = 20) -> str:
        """(For HPC, Linux), generates an ASCII art representation of the equity curve."""
